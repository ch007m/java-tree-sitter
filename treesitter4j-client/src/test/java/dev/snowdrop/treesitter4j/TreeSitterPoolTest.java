...
class TreeSitterPoolTest {
    ...
    @Test
    void parseDirectoryIntegration() {
        String dirPath = System.getProperty("ts4j.test.dir");
        if (dirPath == null || dirPath.isBlank()) {
            System.out.println("Skipping directory test — set -Dts4j.test.dir=<path> to enable");
            return;
        }

        var parserUtil = new dev.snowdrop.treesitter4j.util.ASTParserUtil();
        assertDoesNotThrow(() -> {
            var trees = parserUtil.parseDirectory(
                    parserUtil.resolveAppDir(dirPath),
                    System.out::println);
            assertFalse(trees.isEmpty(), "Should parse at least one file from " + dirPath);
        });
    }
}
